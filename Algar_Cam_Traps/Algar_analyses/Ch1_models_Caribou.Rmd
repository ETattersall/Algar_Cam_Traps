...
ER
```
 Examining summaries for top 5 models (2dAIC)  
```{r, echo=FALSE, results='hide'}
fixef(Models$`14`)
fixef(Models$`13`)
fixef(Models$`30`)
...
fixef(Models$`16`)
```  
In addtion to Treatment and ActiveDays, all 5 models contain pSnow and lowland. 3 contain LD, 2 contain VegHt, and 1 contains LineWidth
...
summary(LTint)
```
  Although not significant, this indicates that the main effect of lowland habitat on caribou detections decreases on HumanUse, NatRegen, and SPP lines relative to effect of lowland on control.
