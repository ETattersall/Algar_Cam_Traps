...
```
  Five models within 2 dAIC points of each other, with model weights between 7 - 18%.
...
```{r, echo=FALSE}
LTint <- glmmTMB(Wolf ~Treatment*pSnow_sc + low500_sc + VegHt_sc + ActiveDays_sc + (1 | Site) + (1 | Month), data = det, zi=~1, family = nbinom1)
summary(LTint)
```
  This indicates that the main effect of snow on wolf detections decreases on HumanUse, NatRegen, and SPP lines relative to effect of snow on control. This would mean that the negative effect of snow on wolf detections is slightly offset by HumanUse lines